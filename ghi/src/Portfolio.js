...
function Portfolio() {
    ...
    const displayHair = () => {
        return (
            ...
            <div className="grid grid-cols-2 align-middle items-center w-4/5 h-4/5 sm:w-3/5 sm:h-3/5 mt-20 gap-3 md:gap-7">
                {hairImagesList.map((img, index) => {
                    return (
                    <div key={index}>
                        <button
                        onClick={handleSetImage}
                        type="button"
                        data-modal-target="img-modal"
                        data-modal-toggle="img-modal">
                            <img
                            className="rounded-lg border-2 border-black mb-7"
                            src={img}></img>
                        </button>
                    </div>
                    );
    ...
    // console.log("REF *****", hiddenFileInput);
...
"text-xl bg-pink-500 py-2 px-4 shadow-2xl font-serif border-2 border-black rounded-2xl hover:bg-pink-400 mx-5 text-black font-bold" */}
        ...
        : null
                                                                                                                    ...
                                                                                                                    {/* bg-gray-600 */}
        {displayModal
        ? <div tabIndex="-1" className="flex flex-col gap-3 border border-black shadow-2xl w-5/6 h-3/5 max-h-[500px] md:w-1/2 md:h-1/2 lg:w-2/3 lg:h-4/6 lg:max-h-[700px] max-w-[600px] top-0 left-0 right-0 bottom-0 z-50 bg-gray-900 mx-auto my-auto fixed rounded md:inset-0 justify-center items-center">
            <div className="flex flex-row w-full justify-evenly border-b border-neutral-500">
                <h1 className="text-4xl lg:text-7xl text-white font-bold font-portfolio">Simply Chic</h1>
                <button onClick={handleCloseModal} className="flex flex-row md:ml-20"><AiOutlineCloseCircle className="h-8 w-8 sm:h-12 mt-3 sm:w-12 mb-3 ml-20 justify-end text-white hover:rotate-90 hover:text-red-600 transition-all duration-300" /></button>
                {/* <hr /> */}
